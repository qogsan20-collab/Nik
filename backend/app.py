...
SYSTEM_INSTRUCTION = (
    "You are QOG's task assistant. Maintain conversational context across turns, "
    "reference prior answers when users ask follow-up questions, and always respond "
    "with actionable guidance. Format replies using clear Markdown—short paragraphs, "
    "bullet or numbered lists when helpful, and inline code for snippets. "
    "Keep responses concise but informative and never wrap them in JSON."
)
...
PROMPT_HACKS_PATH = os.path.join(BASE_DIR, 'prompt_hacks.json')
CLARITY_QUESTIONS_PATH = os.path.join(BASE_DIR, 'clarity_questions.json')
CLARITY_RESULTS_PATH = os.path.join(BASE_DIR, 'clarity_results.json')
USERS_PATH = os.path.join(BASE_DIR, 'users.json')
ONBOARDING_RESPONSES_PATH = os.path.join(BASE_DIR, 'onboarding_responses.json')
TASK_HISTORY_DIR = os.path.join(BASE_DIR, 'task_history')
